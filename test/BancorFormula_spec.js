...
config(
  {
    blockchain: {
      accounts: [
        {
          mnemonic:
            'foster gesture flock merge beach plate dish view friend leave drink valley shield list enemy',
          balance: '5 ether',
          numAddresses: '10',
        },
      ],
...
contract('TestBancorFormula', function() {
  ...
  let MAX_BASE_N = new BigNumber(2)
    .exponentiatedBy(256 - constants.MAX_PRECISION)
    .minus(1)
  ...
  for (
    ...
    let shlVal = new BigNumber(2).exponentiatedBy(
      constants.MAX_PRECISION - precision,
    )
    let tuples = [
      {
        input: maxExp
          ...
          .times(shlVal)
          .minus(1),
        output: new BigNumber(precision - 0),
      },
      {
        input: maxExp
          .plus(0)
      ...
      },
      {
        input: maxExp
          ...
          .times(shlVal)
          .minus(0),
        output: new BigNumber(precision - 1),
      },
    ]
  ...
  for (let n = 1; n <= 255; n++) {
    let tuples = [
      ...
      {
        input: new BigNumber(2).exponentiatedBy(n).plus(1),
        output: new BigNumber(n),
      },
      {
        input: new BigNumber(2).exponentiatedBy(n + 1).minus(1),
        output: new BigNumber(n),
      },
    ]
  ...
  for (let percent = 0; percent < 100; percent++) {
    ...
    it(`Function optimalLog(${x.toFixed()})`, async () => {
      try {
        ...
        let ratio = fixedPoint.isEqualTo(floatPoint)
          ? BigNumber(1)
          : fixedPoint.dividedBy(floatPoint)
        assert(
          ratio.isGreaterThanOrEqualTo(
            '0.99999999999999999999999999999999999',
          ) && ratio.isLessThanOrEqualTo('1'),
          `ratio = ${ratio.toFixed()}`,
        )
      } catch (error) {
        assert(false, error.message)
